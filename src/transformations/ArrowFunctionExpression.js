...
function resolveArrowFunctionExpression(node, options) {
    options = options || {}
    ...
    if (options.declareName) {
        log(new LogData(`Declaring arrow function ${options.declareName} at register ${options.declareRegister}`, 'accent', true))
        this.declareVariable(options.declareName, options.declareRegister)
    } else {
        log(new LogData(`Declaring anonymous arrow function at register ${options.declareRegister}`, 'accent', true))
    }
...
module.exports = resolveArrowFunctionExpression
